...
if __name__ == "__main__":
    ...
    #tcrex_preds.rename(columns={'CDR3_beta': 'junction_aa'}, inplace=True)
    
    
    ### set BPR threshold
    #tcrex_preds = tcrex_preds[tcrex_preds['bpr'] <= 0.0001]
    
    # replace this
    tcrex_preds = DETECT_predictions
    
    ####################################################################################################################################
    tcrex_merged = pd.merge(tcrex_preds.drop_duplicates(subset=['clonotype']), data, on=['clonotype'], how='inner')
    tcrex_merged['TIMEPOINTS'] = pd.Categorical(tcrex_merged['TIMEPOINTS'], categories=['baseline', 'V1', 'V3',], ordered=True)
    tcrex_merged.drop_duplicates(subset=['sample_id', 'clonotype'], inplace=True)
    ...
    tcrex_merged = tcrex_merged[['sample_id', 'cloneFraction', 'CONDITION', 'TIMEPOINTS']].groupby(['sample_id','CONDITION', 'TIMEPOINTS'], as_index=True).agg({'cloneFraction': 'sum'})
    tcrex_merged.reset_index(inplace=True)
    tcrex_merged = pd.merge(tcrex_merged, metadata[['sample_id', 'AB_TITER']], on='sample_id', how='left')
    plotting.plot_clonalfraction_antibody(tcrex_merged, PLOTSDIR)
    ####################################################################################################################################
    
    
    ####################################################################################################################################
    # per epitope analysis
    tcrex_merged = pd.merge(tcrex_preds, data, on='clonotype', how='inner')
    tcrex_merged['TIMEPOINTS'] = pd.Categorical(tcrex_merged['TIMEPOINTS'], categories=['baseline', 'V1', 'V3',], ordered=True)
    tcrex_merged.dropna(inplace=True)
    #plotting.plot_heatmap_tcrex_predictions(tcrex_merged, PLOTSDIR)
    ...
    scatteratio_data.drop_duplicates(subset=['sample_id', 'clonotype', 'TIMEPOINTS'], inplace=True)
    ...
    opacities = [1 if col in tcrex_clonotypes else 0.1 for col in clone_fractions_pivoted.columns]
    
    print(clone_fractions_pivoted)
    exit(0)
