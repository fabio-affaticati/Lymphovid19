...
if __name__ == "__main__":
    ...
    ####################################################################################################################################
    scatteratio_data = pd.merge(data, DETECT_predictions.drop_duplicates(subset=['clonotype']), on=['clonotype', 'junction_aa', 'j_call', 'v_call'], how='left', indicator=True)
    scatteratio_data.drop_duplicates(subset=['sample_id', 'clonotype', 'TIMEPOINTS'], inplace=True)
    
    # This selects only beta chains
    #scatteratio_data = scatteratio_data.query('TCR_chain == TRB')
